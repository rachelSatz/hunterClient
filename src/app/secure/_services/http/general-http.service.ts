...
export class GeneralHttpService extends BaseHttpService {
  ...
  getEmployerTypeSentEnum(token: string): Promise<{Key: string, Value: number}[]> {
    return this.http.get(this.apiUrl  + '/employerTypeSent', this.getTokenHeader(token)).toPromise()
      .then(response => response as {Key: string, Value: number}[]);
  }
